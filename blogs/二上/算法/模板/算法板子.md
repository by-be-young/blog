...
每个物品能选任意次（洛谷P 244，需要修改的部分见P 243）

实际上，只需要改变内层循环的遍历方向即可。
...
const int N = 1010;
int n, m, k;
vector<pair<int, int>> groups[N];  // 存储每组的物品(重量,价值)
int f[N];

int main() {
    cin >> m >> n;  // m:背包容量, n:物品数量
    ...
    for (int g = 1; g <= k; g++)  // 遍历所有组
        for (int j = m; j >= 0; j--)  // 容量逆序遍历
            for (auto& item : groups[g])  // 遍历组内物品
                if (j >= item.first)
    ...
    for (int i = 1; i <= l1; i++) {
        ...
        for (int j = 1; j <= l2; j++) {
            if (word1[i - 1] == word2[j - 1]) {
                ...
                int temp = dp[0][j - 1] < dp[0][j] ? dp[0][j - 1] : dp[0][j];
                temp = temp < dp[1][j - 1] ? temp : dp[1][j - 1];
                dp[1][j] = temp + 1;
            }
        }
